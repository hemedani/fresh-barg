...
export const CreateRoleModal: React.FC<CreateRoleModalProps> = ({
        ...
        const fetchData = async () => {
            ...
            try {
                const [orgRes, unitRes, userRes] = await Promise.all([
                    getOrgans({ get: { _id: 1, name: 1 }, set: { page: 1, limit: 50, positionId } }),
                    getUnits({ get: { _id: 1, name: 1 }, set: { page: 1, limit: 50, positionId } }),
                    getUsers({ get: { _id: 1, first_name: 1, last_name: 1 }, set: { page: 1, limit: 100 } }),
                ]);
    ...
    return (
        <Modal isOpen={isOpen} onClose={handleClose} title="ایجاد نقش جدید" className="max-w-4xl">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-h-[70vh] overflow-y-auto p-1">
                ...
                <div className="flex justify-end gap-3 pt-4 border-t border-slate-700">
                    <Button type="button" onClick={handleClose} className="bg-slate-600 hover:bg-slate-700 text-white px-6 py-2">
                        لغو
                    </Button>
                    <button
                        type="submit"
